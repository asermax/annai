...
export const parseStartArgs = (argv) => {
    let surfacePath;
    let sessionId;
    let repoPath;
    let noOpen = false;
    for (let i = 0; i < argv.length; i++) {
        const arg = argv[i];
        if (arg === '--surface' && i + 1 < argv.length) {
            surfacePath = argv[i + 1];
            i++;
            continue;
        }
        ...
        if (arg === '--port' && i + 1 < argv.length) {
            i++;
            continue;
        } // accepted, currently ignored (auto)
        throw new Error(`start: unknown argument "${arg}"`);
    }
    if (surfacePath == null)
        throw new Error('start: --surface <path> is required');
    if (sessionId == null)
        throw new Error('start: --session <id> is required');
...
export const runStart = async (argv) => {
    ...
    if (child.pid == null)
        throw new Error('start: failed to spawn daemon');
    ...
    if (state.port == null)
        throw new Error('start: daemon state has no port');
