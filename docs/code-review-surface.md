...
- **Draft PR comments** — reviewer can leave queued comments on the surface that get sent to the PR when ready (target a `diffs.com` / `diff2html`-style library for the final product)
...
- **Annotation library**: target the `diff2html` / `diffs.com` family for diff rendering plus comment overlay in the final product.
...
- diff2html / diffs.com — diff-rendering library family for final implementation
